    ...
    if params.with_webkit:
        ...
        if machinery.IS_QT6:
            monkeypatch.setattr(
                profiles,
                "default_profile",
                lambda: FakeExtensionProfile(FakeExtensionManager([FakeExtensionInfo("ext1")])),
            )
...
class TestWebEngineExtensions:
    ...
    def test_qtwebkit(self, monkeypatch: pytest.MonkeyPatch) -> None:
        assert profiles.default_profile() is None  # -> default_qt_profile() used
    ...
    def test_preexisting_profile(self, monkeypatch: pytest.MonkeyPatch, avoid_init: bool) -> None:
        ...
        monkeypatch.setattr(profiles, "default_profile", lambda: FakeExtensionProfile(FakeExtensionManager([])))
        if avoid_init:
            monkeypatch.setattr(objects, "debug_flags", {"avoid-chromium-init"})
    ...
    def test_extensions(
        ...
        monkeypatch.setattr(
            profiles,
            "default_profile",
            lambda: FakeExtensionProfile(FakeExtensionManager(extensions)),
        )
